USE `oee`;
DROP procedure IF EXISTS `prc_machine_week_day_report_chart`;

DELIMITER $$
USE `oee`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_machine_week_day_report_chart`(
	IN p_channel_id int,
		  ...
		  from (
			select mpd.date_ref
			 ...
			 where mpd.channel_id = __channel_id
			   and mpd.machine_code = \'__machine_code\'
			   __period

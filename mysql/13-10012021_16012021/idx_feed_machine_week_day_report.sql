...
alter table feed add column inserted_at_month int GENERATED ALWAYS AS (month(`inserted_at`)) STORED;
alter table feed add column inserted_at_week int GENERATED ALWAYS AS (week(`inserted_at`, 5)) STORED;
alter table feed add column inserted_at_day int GENERATED ALWAYS AS (day(`inserted_at`)) STORED;
